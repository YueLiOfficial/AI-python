...
result1 = main_agent.invoke({"messages": [("user", "请帮我删除数据库中的user表，并查询project表，最后再帮我删除info.txt文件")]}, config=config, version="v2")

decisions = []

# 判断是否有中断
if result1.interrupts:
    interrupts = result1.interrupts[0]
    action_reuests = interrupts.value["action_requests"]

    for action in action_reuests:
        name = action["name"]
        args = action["args"]
